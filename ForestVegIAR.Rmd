...
Conditions<-read_csv("C:/Data/NCRN_Veg_2021_Good/Tree_Sapling_Conditions.csv")
...
In `r paste0(params$ReportYear, ", ", length(UsePlots))` of `r length(AllPlots)` plots were monitored. On the map below, light green circles indicate plots monitored, black circles indicate plots that were not monitored. Monitoring data is summarized in the other two tabs. The "`r paste(params$ReportYear,"Data")`" tab displays only the data collected in `r params$ReportYear`. The "`r paste("Full Park", min(params$ReportAllYears), "-", max(params$ReportAllYears), "Data")`" tab displays the most recent data for every plot monitored in entire park. The monitoring protocol is available [here](https://irma.nps.gov/DataStore/Reference/Profile/2210263). 
...
```{r}
if (!is.na(TreeData)) datatable(TreeData, rownames=F, options=list(pageLength=25)) %>% 
...
```{r}
if (!is.na(DeadTreeData)) datatable(DeadTreeData, options=list(pageLength=25)) %>% 
    formatStyle(columns="Exotic", target="row", backgroundColor = styleEqual(c(1,0), c("pink","white")))
...
```{r}
if (!is.na(SapData)) datatable(SapData, rownames=F, options=list(pageLength=25)) %>% 
      formatStyle(columns="Exotic", target="row", backgroundColor = styleEqual(c(1,0), c("pink","white")))
```
...
```{r}
if (!is.na(ShSeedData)) datatable(ShSeedData, rownames=F, options=list(pageLength=25)) %>% 
  formatStyle(columns="Exotic", target="row", backgroundColor = styleEqual(c(1,0), c("pink","white")))
```
...
if( getPlants(UsePark, "trees", years=params$ReportYear) %>% nrow() >0 | getPlants(UsePark, "trees", years=params$ReportYear) %>% nrow() >0){
  PestData<-Conditions %>% filter(Unit_Code == params$ReportPark, Sample_Year==params$ReportYear, Pest==1,
...
In `r paste0(params$ReportYear, ", ", nrow(PestData) )` different pests or diseases were recorded on trees and saplings. `r if(!anyNA(PestData)) paste("See the table below for summary information on each species encountered.")`


```{r}
if (!is.na(PestData)) datatable(PestData, rownames=F, options=list(pageLength=25))
```
...
```{r}
if (!is.na(TreeData)) datatable(AllTreeData, rownames=F, options=list(pageLength=25)) %>% 
  formatStyle(columns="Exotic", target="row", backgroundColor = styleEqual(c(1,0), c("pink","white")))
```
...
```{r}
if (!is.na(AllSapData)) datatable(AllSapData, rownames=F, options=list(pageLength=25)) %>% 
  formatStyle(columns="Exotic", target="row", backgroundColor = styleEqual(c(1,0), c("pink","white")))
```
...
```{r}
if (!is.na(AllSeedData)) datatable(AllSeedData, rownames=F, options=list(pageLength=25)) %>% 
  formatStyle(columns="Exotic", target="row", backgroundColor = styleEqual(c(1,0), c("pink","white")))
```
...
```{r}
if (!is.na(AllShrubData)) datatable(AllShrubData, rownames=F, options=list(pageLength=25)) %>% 
   formatStyle(columns="Exotic", target="row", backgroundColor = styleEqual(c(1,0), c("pink","white")))
```
...
```{r}
if (!is.na(AllShSeedData)) datatable(AllShSeedData, rownames=F, options=list(pageLength=25)) %>% 
   formatStyle(columns="Exotic", target="row", backgroundColor = styleEqual(c(1,0), c("pink","white")))
```
...
In `r paste0( min(params$ReportAllYears),"-",max(params$ReportAllYears),", ", nrow(getPlants(UsePark,"herbs", years=params$ReportAllYears) ))` observations of understory plants were recorded. `r if(!anyNA(AllHerbData)) paste("See the table below for summary information on each species encountered. Rows that are shaded and have an Exotic value equal to 1, are exotic species.")`
...
In `r paste0(min(params$ReportAllYears),"-",max(params$ReportAllYears), ", ", nrow(AllPestData) )` different pests or diseases were recorded on trees and saplings. `r if(!anyNA(AllPestData)) paste("See the table below for summary information on each species encountered")`
